    ...
    const buildEditForm = async (data) => {
                    ...
                    return `
                        <li class="attachment-item" id="attachment-${att.id_attachment}">
                            <div class="attachment-preview">
                                ${isImage ? `<img src="${API_BASE_URL}/uploads/${att.file_path}" alt="preview">` : `<div class="attachment-preview-icon">📄</div>`}
                            </div>
        ...
        switch (type) {
                ...
                formHTML = `
                    ...
                    <div class="form-group"><label>Status Verifikasi</label>
                        <select name="status_verifikasi">
                            ...
                            <option value="Belum terverifikasi" ${data.status_verifikasi === 'Belum terverifikasi' ? 'selected' : ''}>Belum terverifikasi</option>
                        </select>
                    </div>
                    ${attachmentHTML(data)}`;
                break;
            case 'po':
                ...
                break;
            case 'memo':
                formHTML = `<div class="form-group"><label>Perihal</label><textarea name="perihal" required>${data.perihal || ''}</textarea></div>${attachmentHTML(data)}`;
                break;
            case 'invoice':
                ...
                formHTML = `
                    ...
                    <div class="form-group"><label>Purchase Order (PO)</label><select name="no_po"><option value="">- Pilih PO -</option>${poOptions}</select></div>
                    ...
                    <div class="form-group"><label>Status PPN</label>
                        <select name="ppn_status"><option value="PKP" ${data.ppn_status === 'PKP' ? 'selected' : ''}>PKP</option><option value="Non PKP" ${data.ppn_status === 'Non PKP' ? 'selected' : ''}>Non PKP</option></select>
                    </div>
                    <div class="form-group"><label>Status Invoice</label>
                        <select name="status_invoice"><option value="Unbill" ${data.status_invoice === 'Unbill' ? 'selected' : ''}>Unbill</option><option value="Bill" ${data.status_invoice === 'Bill' ? 'selected' : ''}>Bill</option><option value="Paid" ${data.status_invoice === 'Paid' ? 'selected' : ''}>Paid</option></select>
                    </div>`;
                break;
            case 'client':
                formHTML = `
